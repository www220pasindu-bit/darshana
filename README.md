...
<body>
  ...
  <script>
    ...
    function initCalendar(){
      ...
      if(saved){
        saved.forEach(s => {
          document.getElementById('in-'+s.date).innerText = s.checkIn;
          document.getElementById('out-'+s.date).innerText = s.checkOut;
          document.getElementById('late-'+s.date).innerText = s.late;
        });
      }
    }
    ...
    function checkIn(){
      ...
      const late = now.getHours() - shiftStart;
      shifts[today-1].late = late > 0 ? late*60 + now.getMinutes() : 0;
