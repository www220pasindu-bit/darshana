...
.offDay {background-color: #f44336; color:#fff;}
.poya {background-color: #ffa726; color:#000;}
.attended {background-color: #ffeb3b; color:#000;}
...
<h2>Shift Tracker (2 Day / 2 Night)</h2>
...
function generateSchedule(){
  ...
  for(let i=0;i<30;i++){
    ...
    const isPoya = poyaDates.includes(d.toISOString().split('T')[0]);
    
    attendanceData.push({checkIn:'',checkOut:'',late:0,off:shiftType==='Off',poya:isPoya});
    
    const tr = document.createElement('tr'); tr.id='row'+i;
    tr.className = shiftClass;
    if(isPoya) tr.classList.add('poya');
